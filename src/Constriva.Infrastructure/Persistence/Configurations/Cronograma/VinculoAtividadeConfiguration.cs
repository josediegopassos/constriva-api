...
public class VinculoAtividadeConfiguration : IEntityTypeConfiguration<VinculoAtividade>
{
    public void Configure(EntityTypeBuilder<VinculoAtividade> b)
    {
        ...
        b.HasOne(e => e.AtividadeOrigem).WithMany(a => a.Predecessoras).HasForeignKey(e => e.AtividadeOrigemId).OnDelete(DeleteBehavior.Restrict);
        b.HasOne(e => e.AtividadeDestino).WithMany(a => a.Sucessoras).HasForeignKey(e => e.AtividadeDestinoId).OnDelete(DeleteBehavior.Restrict);
    }
}
