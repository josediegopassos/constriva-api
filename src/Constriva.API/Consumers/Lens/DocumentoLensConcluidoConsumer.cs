...
namespace Constriva.API.Consumers.Lens;
        ...
        if (!string.IsNullOrEmpty(evento.DadosExtraidosJson))
        {
            try
            {
                ...
                documento.ValorTotalExtraido ??=
                    GetDec(root, "total_invoice")
                    ?? GetDec(root, "service_value")
                    ?? GetDec(root, "net_value")
                    ?? GetDec(root, "total")
                    ?? GetDec(root, "value")
                    ?? GetDec(root, "charged_value")
                    ?? GetDec(root, "gross_value")
                    ?? GetDec(root, "total_service_value")
                    ?? GetDec(root, "current_total")
