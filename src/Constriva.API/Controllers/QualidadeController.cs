...
public sealed class QualidadeController : BaseController
{
    ...
    public async Task<IActionResult> CreateInspecao([FromBody] CreateInspecaoDto dto, CancellationToken ct)
    {
        try
        {
            return Ok(await Mediator.Send(new CreateInspecaoCommand(RequireEmpresaId(), dto), ct));
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> UpdateInspecao(Guid id, [FromBody] UpdateInspecaoDto dto, CancellationToken ct)
    {
        try
        {
            return Ok(await Mediator.Send(new UpdateInspecaoCommand(id, RequireEmpresaId(), dto.NumeroInspecao, dto.DataProgramada, dto.DataRealizada, dto.Responsavel, dto.Observacoes, dto.Status.ToString()), ct));
        }
        catch (Exception ex)
        {
            return HandleException(ex);
    ...
    public async Task<IActionResult> DeleteInspecao(Guid id, CancellationToken ct)
    {
        try
        {
            await Mediator.Send(new DeleteInspecaoCommand(id, RequireEmpresaId()), ct);
            return NoContent();
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> CreateNC([FromBody] CreateNCDto dto, CancellationToken ct)
    {
        try
        {
            return Ok(await Mediator.Send(new CreateNCCommand(RequireEmpresaId(), dto), ct));
        }
        catch (Exception ex)
        {
            return HandleException(ex);
    ...
    public async Task<IActionResult> UpdateNC(Guid id, [FromBody] UpdateNCDto dto, CancellationToken ct)
    {
        try
        {
            return Ok(await Mediator.Send(new UpdateNCCommand(id, RequireEmpresaId(), dto.Descricao, dto.Causa, dto.AcaoCorretiva, dto.DataPrazo, dto.Status.ToString()), ct));
        }
        catch (Exception ex)
        {
            return HandleException(ex);
    ...
    public async Task<IActionResult> DeleteNC(Guid id, CancellationToken ct)
    {
        try
        {
            await Mediator.Send(new DeleteNCCommand(id, RequireEmpresaId()), ct);
            return NoContent();
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> UpdateFVS(Guid id, [FromBody] UpdateFVSDto dto, CancellationToken ct)
    {
        try
        {
            return Ok(await Mediator.Send(new UpdateFVSCommand(id, RequireEmpresaId(), dto.Servico, dto.Responsavel?.ToString(), dto.DataVerificacao, dto.Aprovado, dto.Observacoes), ct));
        }
        catch (Exception ex)
        {
            return HandleException(ex);
    ...
    public async Task<IActionResult> DeleteFVS(Guid id, CancellationToken ct)
    {
        try
        {
            await Mediator.Send(new DeleteFVSCommand(id, RequireEmpresaId()), ct);
            return NoContent();
        }
        catch (Exception ex)
        {
